...
#define LEDmouseSelect RC1
#define LedCapsLock	RC2
...
void main(){
	...
	TRISC	= 0b10111000; // set all as the inputs and RC6 as output for rs232 transmition and RC0 as output _blinking led
		...
		{			
			...
			if(CAPSLOCK == 1){
				...
				if( POS_1!=0 ){
					...
					else if( prevChar=='1' ){	KeySend('^');	KeySend('.');	prevChar='.';	counter = 10;	}
					else if( prevChar=='.' ){	KeySend('^');	KeySend(',');	prevChar=',';	counter = 10;	}
					else if( prevChar==',' ){	KeySend('^');	KeySend('(');	prevChar='(';	counter = 10;	}
					else if( prevChar=='(' ){	KeySend('^');	KeySend(')');	prevChar=')';	counter = 10;	}
					else if( prevChar==')' ){	KeySend('^');	KeySend('1');	prevChar='1';	counter = 10;	}
				...
				//**** WXYZ ****//	
					...
					else if( prevChar=='0' ){	KeySend('^');	KeySend(' ');	prevChar=' ';	counter = 10;	}
				}	
				else if( POS_12!=0 ){
					// this delay for remove the debouncing
					__delay_ms(50);	while(POS_1!=0); __delay_ms(30);	while(POS_1!=0);	__delay_ms(30);	while(POS_1!=0);	__delay_ms(30);	while(POS_1!=0);
					...
					//if ( counter==0 ){	KeySend('$');	prevChar='0';	counter = 10;	}
					CAPSLOCK = !CAPSLOCK ;	
					LedCapsLock	= CAPSLOCK;
				}
			}
			else{
