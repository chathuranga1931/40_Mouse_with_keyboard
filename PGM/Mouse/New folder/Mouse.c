...
Author 			: 	Chathuranga
Date 			: 	2015.06.05
Descrioption	: 	This code functioning mouse very closely as mouse 
					one segment	setted to run with the various angles
					and others are only functionig with 45 degree 
					angles.
...
{ 
	...
	{	a2d_read();	
		...
		//1
		while( vblAccX < Xlow && vblAccY > Yhigh )
		{
			
			if( vblAccY-Yhigh > Xlow-vblAccX)
			{
				angle =  (vblAccY-Yhigh)/(Xlow-vblAccX);
				
				for(int i=0; i<angle; i++)
				{
					//up
					Y1=1;
					for(int du=1; du<=Range-(vblAccY-Yhigh); du++) delay;
					Y2=1;
					__delay_us(20);
					Y1=0;
					for(int du=1; du<=Range-(vblAccY-Yhigh); du++) delay;
				...
				//left
				X1=1;
				for(int du=1; du<=Range-(Xlow-vblAccX); du++) delay;
				X2=1;
				__delay_us(20);
				X1=0;
				for(int du=1; du<=Range-(Xlow-vblAccX); du++) delay;
			...
			{
				angle =  (Xlow-vblAccX)/(vblAccY-Yhigh);
				for(int i=0; i<angle; i++)
				{
					//left
					X1=1;
		...
		//2
		while( vblAccX > Xhigh && vblAccY > Yhigh )
		{
			
			//right
			X2=1;
			for(int du=1; du<=Range-(vblAccX-Xhigh); du++) delay;
			X1=1;
			__delay_us(20);
			X2=0;
			for(int du=1; du<=Range-(vblAccX-Xhigh); du++) delay;
		...
		//3
		while( vblAccX < Xlow && vblAccY < Ylow )
		{
			...
			__delay_us(20);	
					...
					//down
			Y2=1;
			for(int du=1; du<=Range-(Ylow-vblAccY); du++) delay;
			Y1=1;
			__delay_us(20);
			Y2=0;
			for(int du=1; du<=Range-(Ylow-vblAccY); du++) delay;
		...
		//4
		while( vblAccX > Xhigh  && vblAccY < Ylow )
		{
		...
		//5
		while( vblAccX > Xlow && vblAccX < Xhigh  && vblAccY > Yhigh )
		{
		...
		//7
		while( vblAccX > Xlow && vblAccX < Xhigh  && vblAccY < Ylow )
		{
		...
		//6
		while( vblAccY > Ylow && vblAccY < Yhigh  && vblAccX > Xhigh )
		{
			//right
			X2=1;
			for(int du=1; du<=Range-(vblAccX-Xhigh); du++) delay;
		...
		//8
		while( vblAccY > Ylow && vblAccY < Yhigh  && vblAccX < Xlow )
		{
			//left
			X1=1;
			for(int du=1; du<=Range-(Xlow-vblAccX); du++) delay;
